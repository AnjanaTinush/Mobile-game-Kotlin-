...
RecyclerView.Adapter<QuestionAdapter.ViewHolder>(){
    ...
    val view=LayoutInflater.from(parent.context)
        .inflate(R.layout.activity_item_question_row,parent,false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
       ...
       holder.option1.text=dataSet[position].option1
        ...
        holder.correctAnswer.text="Correct Answar : ${dataSet[position].answer}"

        if(position%2!=0){
            holder.itemView.setBackgroundColor(Color.parseColor("#FFFFFF"))
        }
        else{
            holder.itemView.setBackgroundColor(Color.parseColor("#ebe9e8"))
        }
    }
